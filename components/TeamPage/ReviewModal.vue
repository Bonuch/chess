        ...
        <div class='content-wrapper flex'>
            <div class='left w-50'>
                ...
                <div class='profile-avatar'>
                    ...
                    <img v-if="trainer.teacher_img" :src='trainer.teacher_img' alt=''>
                </div>
                <h2 class='profile-name' v-if="trainer.full_name">{{ trainer.full_name }}</h2>
                <h2 class='profile-name' v-if="trainer.teacher">{{ trainer.teacher }}</h2>
    ...
    methods: {
        ...
        async sendReview() {
            ...
            try {
                ...
                if(this.trainer.teacher_id) {
                    const data = await this.$axios.$post(`/rating/${this.trainer.teacher_id}/vote`, payload)
                    if (data.success) {
                        this.updateDataFn(this.trainerUpdateKey, data.data)
                ...
                if (response.data && response.data.error) {
                    if (response.data.message === 'vote_error_time') {
                        ...
                        this.updateDataFn(this.trainerUpdateKey, this.trainer)
                    }
                }
            }
        }
